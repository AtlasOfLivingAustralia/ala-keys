...
class AuditService {
     ...
     */
    public def logGormEvent(AbstractPersistenceEvent event) {
        ...
        try {
            ...
            map.keySet().each { key ->
                if (!EXCLUDED_ENTITY_PROPERTIES.contains(key)) {
                    props[key] = map[key].toString()
                    if (props[key].toString().length() > 254) props[key] = props[key].toString().substring(0, 254)
                }
            }
